...
<#
.SYNOPSIS
    One-time backfill of historical GitHub Releases for Simple Time-lapse.
...
.DESCRIPTION
    ...
    --latest=false so none of them steals "Latest" from the next real release (stl-v2.0.0+).
...
# 1.0.0 was published under the mod's old name and no zip of it survives locally, so it has no
# release here: a release with no artifact carries less than the changelog already does.
$releases = @(
    @{ artifact='stl'; version='1.1.0'; zip='simple_timelapse_v1.1.0.zip'; changelog=$changelog }
    @{ artifact='stl'; version='1.2.0'; zip='simple_timelapse_v1.2.0.zip'; changelog=$changelog }
    @{ artifact='stl'; version='1.3.0'; zip='simple_timelapse_v1.3.0.zip'; changelog=$changelog }
)
