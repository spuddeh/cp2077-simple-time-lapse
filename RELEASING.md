...
| --- | --- | --- | --- |
| `stl` | Simple Time-lapse | 26107 | main |
...
2. **File id — a repository VARIABLE, not a secret, and not in this repo.**
   ...
   > the void.
   >
   > This mod has published files already, so the id exists and only needs reading.
   >
...
2. Create a GitHub Release whose **tag** follows `<artifact>-v<version>`:
   ```pwsh
   gh release create stl-v2.0.0 --title "Simple Time-lapse v2.0.0" --notes "..."
   ```
...
3. On publish, the workflow:
   ...
   - stages `contentDir` -> `installDir` and zips it as `<fileBaseName>_v<version>.zip` (e.g. `simple_timelapse_v2.0.0.zip`),
...
## The v2.0.0 release body

v2.0.0 adds two requirements a v1.3.0 user does not have, so the file description earns its 255
characters. The body after the marker is **plain lines, one per change** — no headings, no bullet
markers, no `**`. Pull them from `nexus_changelog.md`.
...
- `show_requirements_pop_up: true` shows the requirements popup on download. This mod requires CET,
  Window Utils and Codeware, with XUtils optional.
