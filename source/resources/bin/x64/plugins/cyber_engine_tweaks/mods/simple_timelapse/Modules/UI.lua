...
function UI.Draw(mod, Core, HudUtils, CameraUtils)
    ...
    ImGui.SetNextWindowSizeConstraints(700, 960, 4000, 4000)
    ...
    if ImGui.Begin("Simple Time-lapse", true) then
        ...
        if ImGui.BeginTabBar("MainTabs") then
            ...
            if ImGui.BeginTabItem(" " .. IconGlyphs.Cog .. " Settings ") then
                ...
                local footerHeight = 225
                if mod.settings.useStartTime then footerHeight = 260 end

                -- === BODY (SCROLLABLE) ===
                if ImGui.BeginChild("SettingsBody", 0, -footerHeight, true) then
                    local bodyW = ImGui.GetContentRegionAvail() - 21
                    ...
                    if ImGui.RadioButton("AM", mod.settings.comboAmPm == 0) then mod.settings.comboAmPm = 0 end; ImGui
                        .SameLine()
                    if ImGui.RadioButton("PM", mod.settings.comboAmPm == 1) then mod.settings.comboAmPm = 1 end; ImGui
                        .PopItemWidth()

                    local timeBtnW = (bodyW - (spacing * 3)) / 4
                    if ImGui.Button("6 AM", timeBtnW, 0) then
                        mod.settings.comboHour = 5; mod.settings.comboMinute = 0; mod.settings.comboAmPm = 0; Core
                            .SetTimeNow(mod)
                    end; ImGui.SameLine()
                    if ImGui.Button("12 PM", timeBtnW, 0) then
                        mod.settings.comboHour = 11; mod.settings.comboMinute = 0; mod.settings.comboAmPm = 1; Core
                            .SetTimeNow(mod)
                    end; ImGui.SameLine()
                    if ImGui.Button("6 PM", timeBtnW, 0) then
                        mod.settings.comboHour = 5; mod.settings.comboMinute = 0; mod.settings.comboAmPm = 1; Core
                            .SetTimeNow(mod)
                    end; ImGui.SameLine()
                    if ImGui.Button("12 AM", timeBtnW, 0) then
                        mod.settings.comboHour = 11; mod.settings.comboMinute = 0; mod.settings.comboAmPm = 0; Core
                            .SetTimeNow(mod)
                    end
                    ...
                    if ImGui.IsItemHovered() then
                        ImGui.SetTooltip(
                            "Immediately sets the in-game time to the selected Hour/Minute.")
                    end
                    ...
                    mod.settings.startDelay = ImGui.SliderFloat("##Delay", mod.settings.startDelay, 0.0, 30.0, "%.1fs")
                    if ImGui.IsItemHovered() then
                        ImGui.SetTooltip(
                            "Wait this many seconds after pressing Start before the time-lapse actually begins.")
                    end
                    ...
                    local btnW = (bodyW - (spacing * (#quickSpeeds - 1))) / #quickSpeeds
                    for i, s in ipairs(quickSpeeds) do
                        if i > 1 then ImGui.SameLine() end; if ImGui.Button(s .. "x", btnW, 0) then
                            mod.settings.speed =
                                s
                        end
                    end
                    ...
                    if mod.settings.mode == 0 and mod.settings.unlockSpeed and mod.settings.speed > 10.0 then
                        ImGui.Spacing(); ImGui.PushTextWrapPos(0.0); ImGui.TextColored(1, 0, 0, 1,
                            IconGlyphs.Alert ..
                            " WARNING: Simulation speeds above 10x hit engine limits. Time estimate will be inaccurate, and the simulation will be capped at the engine limit."); ImGui
                            .PopTextWrapPos()
                    end
                    ...
                    ImGui.PushItemWidth(halfW); local val, changed = ImGui.InputFloat("##DurInput", mod.ui.durationVal,
                        0.25, 1.0, "%.2f"); if changed then
                        mod.ui.durationVal = math.max(0, val); Core.RecalcDuration(mod)
                    end; ImGui.PopItemWidth()
                    if ImGui.IsItemHovered() then
                        ImGui.SetTooltip(
                            "Total real-time duration for the time-lapse session.\nSet to 0 for Infinite Mode.")
                    end
                    ImGui.SameLine(); ImGui.PushItemWidth(halfW); local unit, unitChanged = ImGui.Combo("##DurUnit",
                        mod.ui.durationUnit, mod.ui.unitLabels, #mod.ui.unitLabels); if unitChanged then
                    ...
                    local function check(label, varName, tooltip)
                        ...
                        mod.settings[varName] = ImGui.Checkbox(label, mod.settings[varName])
                    ...
                    if mod.settings.forceVehicleDilation then
                        curX = 0 -- Reset checkbox wrapping logic
                        ImGui.Spacing()
                        ImGui.TextWrapped(IconGlyphs.Speedometer .. " Vehicle Speed Multiplier")
                        ImGui.SetNextItemWidth(bodyW)
                        local fSpeed, fChanged = ImGui.SliderFloat("##FrenzySpeed", mod.settings.frenzySpeedMult, 1.0,
                            20.0, "%.1fx")
                        if fChanged then mod.settings.frenzySpeedMult = fSpeed end
                        if ImGui.IsItemHovered() then
                            ImGui.SetTooltip(
                                "Controls how fast traffic moves. Decoupled from time dilation speed.\n" ..
            ...
            if ImGui.BeginTabItem(" " .. IconGlyphs.Bug .. " Debug ") then
                ...
                local btnW = (availW - spacing) / 2
                local isStacked = availW < 380
                if isStacked then btnW = -1 end
                local requireSameLine = not isStacked

                -- Row 1: Player Locks
                if ImGui.Button(IconGlyphs.Walk .. " Unlock Movement", btnW, 0) then
                    CameraUtils.UnlockMovement(mod)
                end
                if ImGui.IsItemHovered() then
                    ImGui.SetTooltip(
                        "Manually removes the Movement Lock status effect. Use if you are stuck in place.")
                end

                if requireSameLine then ImGui.SameLine() end

                if ImGui.Button(IconGlyphs.Eye .. " Unlock Camera", btnW, 0) then
                    CameraUtils.UnlockCamera(mod)
                end
                if ImGui.IsItemHovered() then
                    ImGui.SetTooltip(
                        "Manually removes the Camera Lock status effect. Use if you cannot look around.")
                end

                -- Row 2: Overrides
                if ImGui.Button(IconGlyphs.Eye .. " Force Restore HUD", btnW, 0) then
                    HudUtils.ForceRestore(mod)
                end
                if ImGui.IsItemHovered() then
                    ImGui.SetTooltip(
                ...
                if ImGui.Button(IconGlyphs.Camera .. " Force Head Bob", btnW, 0) then
                    CameraUtils.ForceRestore(mod)
                end
                if ImGui.IsItemHovered() then
                    ImGui.SetTooltip(
                        "Resets Additive Camera Motions to 1.0 (Full). Use if the camera feels too static.")
                end
                ...
                if mod.lastRunStats.valid then
                    ...
                    if mod.settings.mode == 0 then
                        ...
                        ImGui.Spacing(); ImGui.PushTextWrapPos(0.0); ImGui.TextDisabled(
                            "Note: A factor of ~8.0 means the engine is running perfectly linear. Lower values indicate the physics engine has hit its simulation speed cap."); ImGui
                            .PopTextWrapPos()
                    else
                        ...
                        ImGui.Spacing(); ImGui.PushTextWrapPos(0.0); ImGui.TextDisabled(
                            "Note: 100% means the clock moved exactly as fast as requested. Lower values mean the script couldn't keep up (unlikely)."); ImGui
                            .PopTextWrapPos()
                    end
                else
                    ImGui.PushTextWrapPos(0.0); ImGui.TextDisabled(
                        "No data available yet. Run the time-lapse to generate stats."); ImGui.PopTextWrapPos()
                end

                ImGui.Spacing(); ImGui.Separator(); ImGui.Spacing()
                local verStr = mod.version; local verWidth = ImGui.CalcTextSize(verStr); local availW = ImGui
